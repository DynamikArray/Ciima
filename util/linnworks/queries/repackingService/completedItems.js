function completedItems() {
  const strSQL = `script=DECLARE @ebayLots NVARCHAR(MAX);
                  ...
                  SELECT
                      ...
                      rp.repackPending as repackPending,
                      rc.repackCompleted as repackCompleted,
                      rcd.repackCompletedDate as repackCompletedDate
                  FROM
                      StockItem si
                  ...
                  LEFT OUTER JOIN (
                    SELECT si.ItemNumber, siep.ProperyValue AS [repackCompleted], siep.pkRowId as 'repackCompletedId'
                    FROM [StockItem] si
                  ...
                  LEFT OUTER JOIN (
                    SELECT si.ItemNumber, siep.ProperyValue AS [repackCompletedDate], siep.pkRowId as 'repackCompletedDateId'
                    FROM [StockItem] si
                    LEFT OUTER JOIN [StockItem_ExtendedProperties] siep ON si.pkStockItemID = siep.fkStockItemID AND siep.ProperyName = 'repackCompletedDate'
                  ) rcd ON rcd.ItemNumber = si.ItemNumber
