...
exports.buildRequestUrl = function(options) {
  ...
  switch (serviceName) {
    case "Finding":
      ...
      else url = "https://svcs.ebay.com/services/search/FindingService/v1"; // no HTTPS support, really?!
      break;

    case "Product":
      if (options.sandbox)
        url =
          "http://svcs.sandbox.ebay.com/services/marketplacecatalog/" +
          serviceName +
          "/v1";
      else
        url =
          "http://svcs.ebay.com/services/marketplacecatalog/" +
          serviceName +
          "/v1"; // no HTTPS support, really?!
      break;
    ...
    case "Merchandising":
      if (options.sandbox)
        url = "http://svcs.sandbox.ebay.com/MerchandisingService";
    ...
    default:
      if (options.sandbox) {
        throw new Error(
          "Sandbox endpoint for " +
            serviceName +
            " service not yet implemented. Please add."
        );
