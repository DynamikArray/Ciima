  ...
  <div class="dtPerComicPrice w-100">
    ...
    >
      ...
      <template v-slot:item.ItemTitle="{ item }">
        <ItemInfoPanel :item="item" :class="canSubmit(item) ? '' : 'red accent-2 textShadow px-2'" />
      </template>
...
import PerComicPriceSearchForm from "@/components/RepackingService/Tabs/SearchRepacking/dtSearchForm";
...
import BtnSubmitToPending from "./FieldTemplates/BtnSubmitToPending";
...
export default {
  ...
  methods: {
    canSubmit(item) {
      if (item.InOrderBook > 0) return false;
      return true;
    },
    async submitItem({ pkStockItemID, sku }) {
      const res = await this.$store.dispatch(`repackingService/search/${REPACKING_SERVICE_SEARCH_SUBMIT_ITEM}`, {
      ...
      if (res.result && !res.error) {
        ...
        this.$store.dispatch(`repackingService/search/${REPACKING_SERVICE_SEARCH_REMOVE_ITEM}`, {
          pkStockItemId: pkStockItemID
        });
...
<style>
.dtPerComicPrice .dtSlotTop .v-data-footer {
