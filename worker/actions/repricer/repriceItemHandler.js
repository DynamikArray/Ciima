...
const repriceItemHandler = async ({ data }) => {
  ...
  try {
    ...
    if (prices.pricesResult) {
      ...
      const priceSet = prices.pricesResult.filter((item) => item.Tag == "Start").shift();
      ...
      //Handle Item Prices
      const { invPriceResult, invPriceError } = await handleInventoryItemPrice(pkStockItemID, newPrice);
